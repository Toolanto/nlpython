...
def main(argv):
	  ...
	  if (response == 'y' or response == 'Y'):
		...
		nlp.removeStandard(language['i'])
	      ...
	      stopwords = raw_input("Insert the stopwords separate by comma(es: word1,word2,word3): ")
	  ...
	  while (exit):
	    ...
	    if n == 2:
	      token = raw_input("\nEnter token: ")
	      documents = nlp.getDocumentsByToken(token.decode('utf-8').lower())
              if len(documents) == 0 : print "No document satisfies the requirement"
