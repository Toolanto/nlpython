...
class NLP():
  ...
  def calculate_tfidf(self,token,document):
    print self.calculate_tf(token,document)
    print self.calculate_idf(token)
    return self.calculate_tf(token,document)*self.calculate_idf(token)
  
  def findngram(self,ngram):
    return {key:nltk.util.ngrams(doc,ngram) for key,doc in self.corpus.items()}
